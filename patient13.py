...
if platform == 'win32':
    with open("processed_data\\patient13.pickle", "rb") as f:
        lPat, rPat = pickle.load(f)
...
callbacks = [[cBacks.EarlyStoppingAtMinLoss(patience = 20, baseLoss = 0.29),
              cBacks.lrScheduler(refLoss=0.24, gain=0.1)],
             [cBacks.EarlyStoppingAtMinLoss(patience = 20, baseLoss = 0.29),
              cBacks.lrScheduler(refLoss=0.24, gain=0.1)],
             [cBacks.EarlyStoppingAtMinLoss(patience = 20, baseLoss = 0.29),
              cBacks.lrScheduler(refLoss=0.24, gain=0.1)],
...
if platform == 'win32':
    with open("results\\patient13_analysis.pickle", "wb") as f:
