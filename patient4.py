...
if platform == 'win32':
    with open("processed_data\\patient4.pickle", "rb") as f:
        lPat, rPat = pickle.load(f)
...
callbacks = [[cBacks.EarlyStoppingAtMinLoss(patience = 20, baseLoss = 0.20),
              cBacks.lrScheduler(refLoss=0.15, gain=0.1)],
             [cBacks.EarlyStoppingAtMinLoss(patience = 20, baseLoss = 0.20),
              cBacks.lrScheduler(refLoss=0.15, gain=0.1)],
             [cBacks.EarlyStoppingAtMinLoss(patience = 20, baseLoss = 0.20),
              cBacks.lrScheduler(refLoss=0.15, gain=0.1)],
             [cBacks.EarlyStoppingAtMinLoss(patience = 20, baseLoss = 0.20),
...
if platform == 'win32':
    with open("results\\patient4_analysis.pickle", "wb") as f:
