...
def dayPredictionPlot(lPatData,
    ...
    """
    Function to plot a single day of forecasts.
    
    Inputs:
        lPatData - left-arm data to plot
        rPatData - right-arm data to plot
        model - model to plot
        patNumber - which patient to plot
        saveFile - boolean, True or False
        fileName - file name to save if True
    
    Outputs:
        pdf plot saved in fileName location
    """
...
def multiModelEvalPlot(lPats,
    ...
    """
    Function to plot two models to compare the same-arm results.
    
    Inputs:
        lPats - list of left-arm patient instances
        rPats - list of right-arm patient instances
        modelNames - names of models to compare
        labels - data set labels (e.g. left-left)
        index - index for dfs
        patNames - names of patients for plot labels
        savePlot - boolean, true or false
        plotName - name of file to save
    
    Outputs:
        pdf file in location of plotName
    """
...
def timeTrialPlot(data, modelsToPlot, iterationsToPlot, axToPlot):
    """
    Function which plots the training results of multiple models.
    
    Inputs:
        data - dictionary of data with multiple model results to plot
        modelsToPlot - names of models to plot
        iterationsToPlot - which iterations of the time trial to show
        axToPlot - which axes to plot on (generate before function call)
    
    Outputs:
        legendNames - legend names for the plot (model names)
    """
